...
async fn det_descent_target_with_three_default_levels() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let expected = vec![
            TraceEvent::Started { chart_hash: Desc::<8>::CHART_HASH },
            TraceEvent::Entered { state: SR },
            TraceEvent::Entered { state: SI },
            entry(SI, A_IDLE_IN),
            TraceEvent::EventDelivered { handler_state: SI, event: E_GO },
            TraceEvent::TransitionFired { from: Some(SI), to: ST },
            exit_(SI, A_IDLE_OUT),
...
async fn det_descent_entries_fire_outer_to_inner() {
    tokio::task::LocalSet::new().run_until(async {
        ...
        let split = m.journal().iter().position(|e| matches!(
            e, TraceEvent::TransitionFired { .. }
        )).unwrap();
        let entered: Vec<u16> = m.journal()[split..].iter().filter_map(|e| match e {
...
async fn det_descent_action_kind_sequence() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let kinds_after_exit: Vec<ActionKind> = m.journal()[split + 3..].iter().filter_map(|e| match e {
            TraceEvent::ActionInvoked { kind, .. } => Some(*kind),
            _ => None,
        }).collect();
        assert!(kinds_after_exit.iter().all(|k| *k == ActionKind::Entry),
            "all post-transition actions must be Entry kind, got {:?}", kinds_after_exit);
