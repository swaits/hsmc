...
async fn det_parent_timer_armed_before_child_entered() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let armed_idx = j.iter().position(|e| matches!(
            e, TraceEvent::TimerArmed { state: SP, .. }
        )).unwrap();
        let child_entered_idx = j.iter().position(|e| matches!(
            e, TraceEvent::Entered { state: 2 }  // ChildA = id 2
        )).unwrap();
        assert!(armed_idx < child_entered_idx,
...
async fn det_parent_timer_only_one_arm_in_initial_descent() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        let mut m = Pt::new(Ctx);
        let _ = m.dispatch(Ev::Halt).await;
        let armed: Vec<u16> = m.journal().iter().filter_map(|e| match e {
...
async fn det_parent_timer_cancelled_on_termination() {
    tokio::task::LocalSet::new().run_until(async {
        let mut m = Pt::new(Ctx);
        let _ = m.dispatch(Ev::Halt).await;
        let cancels: Vec<&TraceEvent> = m.journal().iter().filter(|e| matches!(
            e, TraceEvent::TimerCancelled { .. }
        )).collect();
        // Termination unwinds all states. Parent's timer is cancelled when Parent exits.
        assert_eq!(cancels.len(), 1,
            "expected exactly 1 TimerCancelled (Parent's), got {:?}", cancels);
        assert!(matches!(
            cancels[0],
            TraceEvent::TimerCancelled { state: SP, timer: T_PARENT }
        ));
    }).await;
}
