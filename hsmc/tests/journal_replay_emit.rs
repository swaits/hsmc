...
async fn emit_queued_appears_in_journal() {
    tokio::task::LocalSet::new().run_until(async {
        ...
        let saw_emit = m.journal().iter().any(|e| matches!(
            e,
            TraceEvent::EmitQueued { .. }
        ));
        assert!(saw_emit, "journal must contain EmitQueued, got {:?}", m.journal());

        // The follow-up event MUST be delivered (proving the queue actually
        // drained the emit'd event).
        let delivered_followup = m.journal().iter().any(|e| matches!(
            e,
            TraceEvent::EventDelivered { event: 1, .. }  // 1 = Followup (Trigger=0, Followup=1)
        ));
        assert!(delivered_followup, "expected Followup to be delivered after emit");
    }).await;
}
