...
async fn det_timer_arms_on_entry() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let first_armed = j
                .iter()
                .find(|e| matches!(e, TraceEvent::TimerArmed { .. }));
            assert!(
                matches!(
                    first_armed,
                    Some(TraceEvent::TimerArmed {
                        state: SC,
                        timer: T_COUNTING,
                        ns: 50_000_000
                    })
...
async fn det_timer_cancels_on_exit() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert!(
                cancelled.is_some(),
                "Counting's timer must be cancelled on exit, got: {:?}",
                j
            );
        })
        .await;
...
async fn det_timer_cancel_appears_before_exit_actions() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let cancel_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::TimerCancelled { state: SC, .. }))
                .unwrap();
            let exit_action_idx = j
                .iter()
                .position(|e| {
                    matches!(
                        e,
...
async fn det_timer_arm_appears_after_entry_actions() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let armed_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::TimerArmed { state: SC, .. }))
                .unwrap();
            assert!(
                entry_idx < armed_idx,
...
async fn det_timer_arm_each_state_in_path() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let armed: Vec<&TraceEvent> = m
                .journal()
                .iter()
                .filter(|e| matches!(e, TraceEvent::TimerArmed { .. }))
                .collect();
            // Only Counting has a timer in initial descent (Done is unreached).
            assert_eq!(
                armed.len(),
                1,
                "expected 1 TimerArmed during initial descent, got {:?}",
                armed
            );
        })
        .await;
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert_eq!(
                evs.len(),
                3,
                "expected arm → cancel → arm (Done's timer), got {:?}",
                evs
            );
            assert!(matches!(
                evs[0],
