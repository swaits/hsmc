...
async fn det_during_started_after_entries() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let w_entry_idx = j.iter().position(|e| {
                matches!(
                    e,
            ...
            let d_started_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::DuringStarted { state: SW, .. }));
            assert!(w_entry_idx.is_some());
            assert!(d_started_idx.is_some());
            assert!(
                w_entry_idx.unwrap() < d_started_idx.unwrap(),
...
async fn det_during_cancelled_before_exits() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let _ = m.dispatch(Ev::Halt).await;
            let j = m.take_journal();
            let d_cancelled_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::DuringCancelled { state: SW, .. }));
            let w_exit_idx = j.iter().position(|e| {
                matches!(
                    e,
                    TraceEvent::ActionInvoked {
                        state: SW,
            ...
            assert!(
                d_cancelled_idx.unwrap() < w_exit_idx.unwrap(),
...
async fn det_during_two_durings_two_started_two_cancelled() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let started = j
                .iter()
                .filter(|e| matches!(e, TraceEvent::DuringStarted { state: SW, .. }))
                .count();
            let cancelled = j
                .iter()
                .filter(|e| matches!(e, TraceEvent::DuringCancelled { state: SW, .. }))
                .count();
            assert_eq!(started, 2);
...
async fn det_during_cancelled_indices_match() {
    tokio::task::LocalSet::new()
        .run_until(async {
            let mut m = Du::new(Ctx::default());
            let _ = m.dispatch(Ev::Halt).await;
            let cancelled_indices: Vec<u16> = m
                .journal()
                .iter()
                .filter_map(|e| match e {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let on_idle: usize = m
                .journal()
                .iter()
                .filter(|e| matches!(e, TraceEvent::DuringStarted { state: SI, .. }))
                .count();
            assert_eq!(on_idle, 0);
        })
        .await;
