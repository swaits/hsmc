...
async fn det_ord_entry_outer_then_inner_across_hierarchy() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            let mut m = Ord::new(Ctx);
            let _ = m.dispatch(Ev::Halt).await;
            let entries_in_order: Vec<u16> = m
                .journal()
                .iter()
                .take_while(|e| !matches!(e, TraceEvent::TerminateRequested { .. }))
                .filter_map(|e| match e {
                    TraceEvent::ActionInvoked {
                        action,
                        kind: ActionKind::Entry,
...
async fn det_ord_actions_then_transition_on_same_trigger() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let after_delivered: Vec<&TraceEvent> = j
                .iter()
            ...
            let handler_actions: Vec<u16> = after_delivered
                .iter()
                .take_while(|e| {
                    matches!(
                        e,
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            assert!(
                ...
                "all exits must precede all entries in a transition"
            );
        })
        .await;
}
