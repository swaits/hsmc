    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let queued = m
                .journal()
                .iter()
                .filter(|e| matches!(e, TraceEvent::EmitQueued { .. }))
                .count();
...
async fn det_emit_followup_event_is_dispatched() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let delivered_events: Vec<u16> = j
                .iter()
...
async fn det_emit_no_reentrant_dispatch() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let queued_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::EmitQueued { event: E_FOLLOWUP }))
                .unwrap();
            let delivered_idx = j
                .iter()
                .position(|e| {
                ...
                .unwrap();
            assert!(
                queued_idx < delivered_idx,
                "EmitQueued ({}) must come before EventDelivered ({})",
                queued_idx,
                delivered_idx
            );
        })
...
async fn det_emit_overflow_records_emitfailed() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let failed = j
                .iter()
                .filter(|e| matches!(e, TraceEvent::EmitFailed { .. }))
                .count();
...
async fn det_emit_state_after_emit_action_runs_to_completion() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let many_delivered_idx = j
                .iter()
                .position(|e| matches!(e, TraceEvent::EventDelivered { event: E_MANY, .. }))
                .unwrap();
            let emit_results: Vec<&TraceEvent> = j[many_delivered_idx..]
                .iter()
                .take_while(|e| {
                    matches!(
                        e,
                ...
                .filter(|e| {
                    matches!(
                        e,
                        TraceEvent::EmitQueued { .. } | TraceEvent::EmitFailed { .. }
                    )
                })
                .collect();
