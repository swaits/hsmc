...
async fn det_self_composite_default_descends_after_re_entry() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let self_idx = j.iter().rposition(|e| matches!(
            e, TraceEvent::TransitionFired { from: Some(SP), to: SP }
        )).unwrap();
        let after = &j[self_idx..];

        let exits: Vec<u16> = after.iter().filter_map(|e| match e {
            TraceEvent::Exited { state } => Some(*state),
            _ => None,
        }).collect();
        let entered: Vec<u16> = after.iter().filter_map(|e| match e {
            TraceEvent::Entered { state } => Some(*state),
            _ => None,
        }).collect();

        assert_eq!(exits, vec![SP], "self-transition exits target only");
        assert_eq!(entered, vec![SP, SC, SL], "re-enter target then default-descend");
    }).await;
}
...
async fn det_self_composite_action_kinds_in_order() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let kinds: Vec<ActionKind> = j[self_idx..].iter().filter_map(|e| match e {
            TraceEvent::ActionInvoked { kind, .. } => Some(*kind),
            _ => None,
        }).collect();
        assert_eq!(kinds,
            vec![ActionKind::Exit, ActionKind::Entry, ActionKind::Entry, ActionKind::Entry],
