...
async fn det_emit_in_exit_action_runs_after_transition() {
    ...
    tokio::task::LocalSet::new().run_until(async {
        ...
        let cleanup_queued = j.iter().position(|e| matches!(
            e, TraceEvent::EmitQueued { event: 0 }
        ));
        let wakeup_queued = j.iter().position(|e| matches!(
            e, TraceEvent::EmitQueued { event: 1 }
        ));
        let cleanup_delivered = j.iter().position(|e| matches!(
            e, TraceEvent::EventDelivered { event: 0, .. }
        ));
        let wakeup_delivered = j.iter().position(|e| matches!(
            e, TraceEvent::EventDelivered { event: 1, .. }
        ));
        ...
        assert!(wakeup_queued.unwrap() < cleanup_delivered.unwrap(),
...
async fn det_emit_in_exit_action_journals_in_order() {
    tokio::task::LocalSet::new().run_until(async {
        ...
        let queued_events: Vec<u16> = j.iter().filter_map(|e| match e {
            TraceEvent::EmitQueued { event } => Some(*event),
            _ => None,
        }).collect();
        assert_eq!(queued_events, vec![0, 1], "Cleanup (id 0) then Wakeup (id 1)");
    }).await;
}
